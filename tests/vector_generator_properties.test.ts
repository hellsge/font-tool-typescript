 ...
 */
const smallCharsetArbitrary = fc
  .array(
...
function readVectorGlyphFromFile(filePath: string, offset: number): VectorGlyphInfo {
  ...
  for (let i = 0; i < totalPoints; i++) {
    const x = buffer.readInt16LE(pos);
    pos += 2;
    const y = buffer.readInt16LE(pos);
    pos += 2;
    points.push({ x, y });
  }
...
describe('Feature: typescript-font-converter, Property 10: Vector Font 包含完整 Glyph 数据', () => {
  ...
  testCondition(
    'should include complete glyph data for all characters',
    async () => {
      await fc.assert(
        fc.asyncProperty(
    ...
    60000
  ); // 60 second timeout for property tests

  testCondition(
    'should include font metrics in header',
    async () => {
      await fc.assert(
        fc.asyncProperty(
          vectorFontConfigArbitrary,
          ...
          async (baseConfig, characters) => {
            ...
            try {
              ...
              const ascent = buffer.readInt16LE(pos);
              pos += 2;
              const descent = buffer.readInt16LE(pos);
              pos += 2;
              const lineGap = buffer.readInt16LE(pos);
              pos += 2;
  ...
  testCondition(
    'should handle empty glyphs gracefully',
    async () => {
...
describe('Vector font generation integration', () => {
  ...
  testCondition(
    'should generate valid output files',
    async () => {
