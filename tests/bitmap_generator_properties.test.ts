 ...
 */
const indexMethodArbitrary = fc.constantFrom(IndexMethod.ADDRESS, IndexMethod.OFFSET);
...
describeIfFontExists('Feature: typescript-font-converter, Property 9: 渲染失败不中断处理', () => {
  ...
  it('should continue processing when some characters fail to render', async () => {
    await fc.assert(
      fc.asyncProperty(characterRangeArbitrary, async ({ start, end }) => {
        const tempDir = createTempDir();
...
describeIfFontExists(
  'Feature: typescript-font-converter, Property 16: Index Array 大小正确性',
  () => {
    /**
     * Property 16: Index Array 大小正确性
     *
...
describeIfFontExists(
  'Feature: typescript-font-converter, Property 17: 未使用索引条目初始化',
  () => {
    /**
     * Property 17: 未使用索引条目初始化
     *
...
describeIfFontExists(
  'Feature: typescript-font-converter, Property 26: 输出文件命名符合规范',
  () => {
    /**
     * Property 26: 输出文件命名符合规范
     *
    ...
    it('should generate correct output filename format', async () => {
      await fc.assert(
        fc.asyncProperty(fontSizeArbitrary, renderModeArbitrary, async (fontSize, renderMode) => {
          const tempDir = createTempDir();
    ...
    it('should use underscore separators in filename', async () => {
      ...
      try {
        const config = createTestConfig({
          ...
          characterSets: [{ type: 'range', value: '0x0041-0x0045' }],
        });
