...
import { validateIndex, IndexValidationResult } from './framework/index-validator';
import { analyzeGlyphSimilarity, GlyphComparisonResult } from './framework/glyph-analyzer';
 ...
 */
function analyzeHeader(cppBinPath: string, tsBinPath: string): DiagnosticResult['header'] {
  try {
    ...
    if (result.match) {
      ...
      if (result.differences && result.differences.length > 0) {
        ...
        for (const diff of result.differences.slice(0, 5)) {
          const offsetStr =
            diff.offset !== undefined ? ` (offset ${formatHex(diff.offset, 2)})` : '';
 ...
 */
function analyzeIndex(tsBinPath: string, header: ParsedHeader): DiagnosticResult['index'] {
  try {
 ...
 */
function analyzeCst(cppCstPath: string, tsCstPath: string): DiagnosticResult['cst'] {
  try {
...
function generateRecommendations(result: DiagnosticResult): string[] {
  ...
  if (result.header.analyzed && !result.header.match) {
    if (result.header.details?.differences) {
      const hasIndexAreaSize = result.header.details.differences.some(
        (d) => d.field === 'indexAreaSize'
      );
      if (hasIndexAreaSize) {
        recommendations.push(
...
async function diagnoseTestCase(testCase: string): Promise<DiagnosticResult> {
  ...
  if (!fs.existsSync(cppDir)) {
    print(
      `\n${COLORS.red}Error: C++ reference directory not found: ${cppDir}${COLORS.reset}`,
      COLORS.red
    );
    return {
      testCase,
  ...
  if (!fs.existsSync(tsDir)) {
    print(
      `\n${COLORS.red}Error: TypeScript output directory not found: ${tsDir}${COLORS.reset}`,
      COLORS.red
    );
    return {
      testCase,
  ...
  print(
    `  C++ binary: ${filesFound.cppBin ? '✓' : '✗'} ${cppBinPath}`,
    filesFound.cppBin ? COLORS.green : COLORS.red
  );
  print(
    `  TS binary:  ${filesFound.tsBin ? '✓' : '✗'} ${tsBinPath}`,
    filesFound.tsBin ? COLORS.green : COLORS.red
  );
  print(
    `  C++ CST:    ${filesFound.cppCst ? '✓' : '✗'} ${cppCstPath}`,
    filesFound.cppCst ? COLORS.green : COLORS.red
  );
  print(
    `  TS CST:     ${filesFound.tsCst ? '✓' : '✗'} ${tsCstPath}`,
    filesFound.tsCst ? COLORS.green : COLORS.red
  );
...
async function main() {
  ...
  if (args[0] === '--all') {
    ...
    const testCases = fs.readdirSync(cppRefDir).filter((name) => {
