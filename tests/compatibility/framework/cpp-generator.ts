...
import {
  loadAllConvertedConfigs,
  CppTestConfig as ConvertedCppTestConfig,
...
export function generateCppReference(
  ...
  const outputBaseDir =
    options?.outputBaseDir || path.resolve(baseDir, DEFAULT_PATHS.CPP_REFERENCE_DIR);
 ...
 */
export function generateAllCppReferences(options?: {
  baseDir?: string;
  configsDir?: string;
  outputBaseDir?: string;
  timeout?: number;
