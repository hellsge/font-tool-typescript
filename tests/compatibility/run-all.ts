...
import { runAllTests, runTestCases, ComparisonResult } from './framework/test-runner';
import {
  generateJsonReport,
  saveJsonReport,
  formatConsoleReport,
...
async function main(): Promise<void> {
  ...
  if (verbose) {
    console.log(
      '\n' +
        formatDetailedConsoleReport(report, {
          colors: true,
          detailed: true,
          showProgress: true,
          maxWidth: 80,
  ...
  console.log(
    `Result: ${report.summary.passed} PASS, ${report.summary.partial} PARTIAL, ${report.summary.failed} FAIL`
  );
