...
const characterSetSourceArbitrary = (): fc.Arbitrary<CharacterSetSource> => {
  return fc.oneof(
    ...
    fc.record({
      ...
      value: fc
        .tuple(fc.integer({ min: 0x0020, max: 0x007f }), fc.integer({ min: 0x0080, max: 0x00ff }))
...
const renderModeArbitrary = (): fc.Arbitrary<RenderMode> => {
  return fc.constantFrom(RenderMode.BIT_1, RenderMode.BIT_2, RenderMode.BIT_4, RenderMode.BIT_8);
};
...
function compareFontConfigs(original: FontConfig, parsed: FontConfig, configDir: string): boolean {
  ...
  return (
    ...
    original.characterSets.every(
      (cs, i) =>
        cs.type === parsed.characterSets[i].type && cs.value === parsed.characterSets[i].value
    )
  );
}
...
describe('Feature: typescript-font-converter, Property 1: 配置解析 Round-Trip', () => {
  ...
  it('should preserve configuration through JSON serialization and parsing', async () => {
    await fc.assert(
      fc.asyncProperty(rootConfigArbitrary(), async (originalConfig) => {
 ...
 */
const invalidFontConfigArbitrary = (): fc.Arbitrary<{
  config: FontConfig;
  invalidField: string;
}> => {
  return fc.oneof(
...
describe('Feature: typescript-font-converter, Property 2: 配置验证拒绝无效输入', () => {
  ...
  it('should reject configurations with invalid values', () => {
    fc.assert(
      fc.property(invalidFontConfigArbitrary(), ({ config, invalidField }) => {
        try {
          ...
          const validCodes = [ErrorCode.CONFIG_VALIDATION_ERROR, ErrorCode.INDEX_METHOD_CONFLICT];
  ...
  it('should accept all valid configurations', () => {
    fc.assert(
      fc.property(fontConfigArbitrary(), (config) => {
        try {
          ConfigManager.validateConfig(config, true); // Skip file checks
          return true;
        } catch (error) {
...
const iniSettingsArbitrary = (): fc.Arbitrary<{ gamma?: number; rotation?: Rotation }> => {
  return fc.record({
    gamma: fc.option(
      fc.integer({ min: 10, max: 300 }).map((n) => n / 100),
      { nil: undefined }
    ),
...
describe('Feature: typescript-font-converter, Property 3: INI 设置覆盖 JSON 配置', () => {
  ...
  it('should override gamma and rotation from INI settings when present', () => {
    fc.assert(
      fc.property(fontConfigArbitrary(), iniSettingsArbitrary(), (config, iniSettings) => {
    ...
    fc.assert(
      fc.property(fontConfigArbitrary(), iniSettingsArbitrary(), (config, iniSettings) => {
        ...
        return (
          ...
          merged.characterSets.every(
            (cs, i) =>
              cs.type === config.characterSets[i].type && cs.value === config.characterSets[i].value
          )
        );
      }),
      { numRuns: 100 }
